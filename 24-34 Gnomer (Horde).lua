Guidelime.registerGuide([[

[N24-34 Phelx's Gnomeregan (Horde) Guide]
[D Phelx's guide to acquiring all Gnomeregean quests. This details the shared & Horde exclusive quests - 5 in total. NOTE: This is for Classic Gnomeregan and may be different for Season of Discovery.]
[GA Horde]

[O] Quests can be obtained from [XP20 20], although to get all you must be [XP25 25], however it's strongly recommended you either run multiple times or wait until you're closer to [XP30 30].
[O] Two of the quests are started from item drops inside ([CI9326] and [CI9308]), with a 3rd starting inside as an escort and ending in Booty Bay.
[O] The two quests inside require 3 Silver each, so be sure to have 6 Silver.
[O] This guide assumes you begin in and have your Hearth set to [S Orgrimmar].
Head to The Valley of Honor and speak to Nogg [G75.8, 25.2 Orgrimmar].
Accept [QA2841], this unlocks another.
Accept [QA2842].
Make your way to Booty Bay, easiest method is fly to [F Ratchet].
Take the boat to Booy Bay, grab the flight path [P Booty Bay].
Speak to Scooty [G27.6, 77.4 Stranglethorn Vale] to turn in [QT2842].
Accept [QA2843], then wait a moment.
Turn in [QT2843].
You have unlocked the teleporter to Gnomeregan!
**Now Enter Gnomeregan**
Keep an eye out for [CI9326] and [CI9308] so you can get [Q2945] and [Q2951], these are completed INSIDE Gnomeregan.
Make a macro **/target Kernobee** to and press it occasionally to find [Q2904].
Complete all the remaining quests.
Hearth to [H Orgrimmar].
Head to Nogg [G75.8, 25.2 Orgrimmar] and turn in [QT2841].
Fly to [F Ratchet].
Take the boat to Booty Bay.
Turn in [QT2904] with Scooty [G27.6, 77.4 Stranglethorn Vale].

]], "Phelx's Dungeon Quest Guide")
